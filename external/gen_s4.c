...
int32_t get_index_s4(const corner_t *corner_positions, const edge_t *edge_positions) {
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 4; j++) {
      if (c_orbit_two_perm == orbit_perm[i][j]) {
        p = i;
      }
    }
  }
  ...
  int32_t c_idx = (c_orbit_perm * c_orbit_two_perm) / p;
...
void generate_stage_four_table(const rubix_cube_t cube) {
  ...
  moves_t lookup[STAGE4_NUM_PERMUTATIONS];
  ...
  permutations[STAGE4_GOAL_STATE] = VISITED;
